...
namespace directx {
	...
	dx_overlay::dx_overlay( std::string target_class, std::string target_window, bool not_topmost ) {
		...
		if ( !FindWindowA( target_class.empty( ) ? NULL : target_class.data( ), target_window.empty( ) ? NULL : target_window.data( ) ) ) {
	...
	void dx_overlay::create_overlay( std::string target_class, std::string target_window ) {
		WNDCLASSEX wc;
		wc.cbSize = sizeof( wc );
		...
		m_target_wnd = FindWindowA( target_class.data( ), target_window.data( ) );
